...
public class BroadcastClient implements Runnable{
	...
	public BroadcastClient(String message, int portNum, int bufferSize, int sleep) {
	...
	public void run() {		
		...
		while(toContinue) {			
			try (DatagramSocket socket = new DatagramSocket()){
				...
				packet = new DatagramPacket(buffer, buffer.length, InetAddress.getByName("255.255.255.255"), portNum);	
				socket.send(packet);
				socket.receive(packet);			
				String msg = (String) BytesUtil.toObject(packet.getData());
				System.out.println("Broadcast client " + message + " recieved: " + msg);
				Thread.sleep(sleep);
			} catch (Exception e) {
				System.err.println("Broadcast client " + message + " crashed");
			}
		}
		System.out.println("Broadcast client " + message + " terminated");
	}
